 ...
 * <br>
 * Event can be received via annotation or {@link org.rschrage.surface.event.EventListener#handleEvent(Event, Object...)}.
 */
public class EventHandler {
    ...
    /**
     * Register an {@link org.rschrage.surface.event.EventListener} for a specific event type.
     *
     * @param eventListener {@link org.rschrage.surface.event.EventListener}, which should get registered
    ...
    /**
     * Unregister an {@link org.rschrage.surface.event.EventListener}.
     *
     * @param eventListener {@link org.rschrage.surface.event.EventListener}, which should get unregistered
     */
    public void unregister(org.rschrage.surface.event.EventListener eventListener, EventType... type) {
        if (eventHandlerMap.containsKey(eventListener)) {
    ...
    /**
     * Checks whether an {@link org.rschrage.surface.event.EventListener} is registered for a specific event type.
     *
     * @param eventListener listener, which should get checked.
     * @return true if the listener is already registered, false otherwise
     */
    public boolean isRegistered(org.rschrage.surface.event.EventListener eventListener, EventType eventType) {
        return eventHandlerMap.containsKey(eventListener)
                && eventHandlerMap.get(eventListener).contains(eventType);
    }

    /**
     * Emits an event to all listeners, which are registered for the specific event type. The events can either be received via {@link org.rschrage.surface.event.EventListener#handleEvent(Event, Object...)} or
    ...
    public void emitEvent(Event event, Object... attachedObjects) {
        for (Map.Entry<EventRouteListener, List<EventType>> entry : eventHandlerMap.entrySet()) {
            ...
            if (sendEvent && !emitReflectionEvent(eventListener, event, attachedObjects)) {
                if (eventListener instanceof org.rschrage.surface.event.EventListener) {
                    ((org.rschrage.surface.event.EventListener)eventListener).handleEvent(event, attachedObjects);
                }
            }
        }
    }
